...
%
% [2] Alois Schl�gl, Oliver Filz, Herbert Ramoser, Gert Pfurtscheller.
	...
	EDF.SIE.RR=1;
        ...
        if EDF.SIE.TECG == 1; 
	        ...
	        if ~isempty(Index)
                ...
                for i_=1:length(EDF.SIE.InChanSelect),
                        k=find(OutChanSelect==EDF.SIE.InChanSelect(i_));
                        if isempty(k)
                                EDF.TECG.QRStemp(:,i_) = zeros(fs,1);
                        else
                                EDF.TECG.QRStemp(:,i_) = QRS.Templates(0.5*fs:1.5*fs-1,k).*hanning(fs);
                        end;
                        [tmp,EDF.TECG.Z(:,i_)] = filter(EDF.TECG.QRStemp(:,i_),1,pulse);
                end;
